...
describe('EvidenceBand', () => {
  ...
  it('renders the exhibit ref + codename and links to the file route', () => {
    renderRouter(<EvidenceBand item={item} index={1} side="right" featured={false} />)
  ...
  it('redacts the configured phrase as a declassifiable control', () => {
    renderRouter(<EvidenceBand item={item} index={1} side="left" featured={false} />)
  ...
  it('flags the featured exhibit', () => {
    renderRouter(<EvidenceBand item={evidence[2]} index={2} side="left" featured />)
...
describe('subject name redaction', () => {
  ...
  it('SubjectBrief redacts the full name and links to the full record', () => {
    ...
    expect(screen.getByRole('button', { name: subjectName.full })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /OPEN FULL RECORD/i })).toHaveAttribute('href', '/record')
  })
})
...
describe('CustodyFooter', () => {
  it('routes to request + ledger and the auth barcode encodes the case code', () => {
    ...
    expect(screen.getByRole('link', { name: /FILE A REQUEST/i })).toHaveAttribute('href', '/request')
    expect(screen.getByRole('link', { name: /CASE LEDGER/i })).toHaveAttribute('href', '/ledger')
    expect(screen.getByRole('img', { name: `Barcode encoding ${bureau.caseCode}` })).toBeInTheDocument()
  })
})
