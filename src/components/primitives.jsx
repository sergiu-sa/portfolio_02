...
export function MarkerFilter() {
  return (
    <svg className="marker-defs" aria-hidden="true" focusable="false" width="0" height="0">
...
export function DeclassifyControl({ className = '' }) {
  ...
  return (
    <div className={`declassify-bar ${allRevealed ? 'is-clear' : ''} ${className}`}>
      <span className="declassify-bar__count">
        CLEARANCE · {String(revealed).padStart(2, '0')}/{String(total).padStart(2, '0')} DECLASSIFIED
      </span>
...
export function DisclosureStamp({ rotate = 6 }) {
  ...
  return (
    <span className="full-disclosure" style={{ '--rot': `${rotate}deg` }} aria-hidden="true">
      FULL DISCLOSURE
    </span>
  )
}
