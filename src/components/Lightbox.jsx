    ...
    >
      <div className="lightbox__top">
        <span className="lightbox__idx">
          EXHIBIT {String(index + 1).padStart(2, '0')} / {String(count).padStart(2, '0')}
        </span>
        <button type="button" className="lightbox__x" data-inspect onClick={onClose} aria-label="Close viewer">
          CLOSE ✕
        </button>
      </div>

      <figure className="lightbox__stage" onClick={(e) => e.stopPropagation()}>
        <img className="lightbox__img" src={item.src} alt={item.caption || `Exhibit ${index + 1}`} />
        {item.caption && <figcaption className="lightbox__cap">{item.caption}</figcaption>}
      </figure>
